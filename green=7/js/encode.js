        ...
        img.onload = function(){
            ...
            for (let i = 0; i < textData.data.length; i += 4) {
                if (textData.data[i+3] !== 0) { //zwart in geheime tekst
                    if (imgData.data[i+1] % 10 == 9) {
                        //doe niets
                    }
                    else if (imgData.data[i+2] > 247) {
                        imgData.data[i+2] = 247; //begrens de waarde
                    }
                    else {
                        while (imgData.data[i+1] % 10 != 9) {
                            imgData.data[i+1]++;//zorg dat op 7 eindigt
                        }
                    }
                    pixelsInMsg++;
                }
